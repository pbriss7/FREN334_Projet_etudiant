...
![](images/OpenRefine.jpg)

Vous aurez ensuite exporté cette table (toujours sous la forme d'un CSV), puis l'aurez déplacé vers le dossier "donnees" de votre environnement Posit. Vous pouvez alors lire ce fichier de la manière suivante:

```{r}
# N'oubliez pas de renommer votre fichier CSV comme suit: "Annotations_pour_dictionnaire_variantes"
...
setnames(dt, 
         old = c("ID", "VARIANTE", "ANCHOR", "NOM_CANONIQUE"),
         new = c("id", "variante", "anchor", "nom_canonique"))

# nrow(dt) == length(unique(dt$ID)) # Simple vérification que tous les ID sont distincts

dt[, .(id, variante, anchor, nom_canonique)] |> kable()
```
...
Importons maintenant le texte du roman *Illusions perdues*. Bien sûr, il faut auparavant avoir placé votre fichier texte (ici: `Illusions_1.txt`) dans le répertoire `donnees`.

```{r}

# Il faut modifier le nom du fichier ci-dessous en fonction du nom de votre propre fichier!
...
Si vous vous travaillez dans Posit-Cloud, vous pouvez exporter vos tables de noeuds et de liens sur votre ordinateur, puis les importer dans Gephi.
